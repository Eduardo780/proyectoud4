...
// Diego García González y Eduardo Álvarez Alonso
    ...
    if (isset($_POST['reservar']) && $id_cliente !== '') {
        if ($id_libro !== '') {
            $check = $conexion->query("SELECT reservado FROM libros WHERE Id = $id_libro AND reservado = 1");
            if ($check->num_rows > 0) {
                ...
                $conexion->query("INSERT INTO reservas (Id, Id_cliente, Id_libro, Fecha_reserva, activa) VALUES ($nuevo_id, $id_cliente, $id_libro, CURDATE(), 1)");
                $conexion->query("UPDATE libros SET reservado = 1 WHERE Id = $id_libro");
                echo "<p><b>Reserva de libro realizada con éxito.</b></p>";
            }
        }
        if ($id_pelicula !== '') {
            $check = $conexion->query("SELECT reservado FROM peliculas WHERE Id = $id_pelicula AND reservado = 1");
            if ($check->num_rows > 0) {
                ...
                $row      = $result->fetch_assoc();
                $nuevo_id = ($row['max_id'] ?? 0) + 1;
                $conexion->query("INSERT INTO reservas (Id, Id_cliente, id_pelicula, Fecha_reserva, activa) VALUES ($nuevo_id, $id_cliente, $id_pelicula, CURDATE(), 1)");
