    ...
    if (isset($_POST['cancelar']) && $id_cliente !== '') {
        if ($id_libro !== '') {
            ...
            if ($check->num_rows === 0) {
                ...
                $conexion->query("DELETE FROM reservas WHERE Id_cliente = $id_cliente AND Id_libro = $id_libro");
        ...
        if ($id_pelicula !== '') {
            ...
            if ($check->num_rows === 0) {
                ...
                $conexion->query("DELETE FROM reservas WHERE Id_cliente = $id_cliente AND Id_pelicula = $id_pelicula");
