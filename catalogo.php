...
// Diego García González y Eduardo Álvarez Alonso
    ...
    if ($mostrar_libros) {
        ...
        if ($titulo   !== '') $cond_libros .= " AND titulo   LIKE '%" . $conexion->real_escape_string($titulo)   . "%'";
        if ($genero   !== '') $cond_libros .= " AND genero   LIKE '%" . $conexion->real_escape_string($genero)   . "%'";
        if ($director !== '') $cond_libros .= " AND autor_id LIKE '%" . $conexion->real_escape_string($director) . "%'";
        if ($anio     !== '') $cond_libros .= " AND YEAR(`Año`) LIKE '%" . $conexion->real_escape_string($anio)  . "%'";

        $consulta_libros  = "SELECT * FROM libros $cond_libros";
